  ...
  return (
    <main className="mx-auto max-w-5xl">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {accesos.map((acceso) => {
                  ...
                  return (
                    <tr key={acceso.empresa_id} className="border-b last:border-0">
                      <td className="px-3 py-4 font-medium">{empresa?.razon_social ?? "Empresa"}</td>
                      <td className="px-3 py-4">{rolLabels[acceso.rol] ?? acceso.rol}</td>
                      <td className="px-3 py-4">{acceso.activo ? "Activo" : "Inactivo"}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <section className="mt-6 rounded-xl border bg-white p-6 shadow-sm">
        <h3 className="text-xl font-semibold">Usuarios y perfiles</h3>
        <p className="mt-2 text-sm text-gray-600">
          La asignación de perfiles se mantiene protegida por las políticas RLS de Supabase.
          Esta pantalla deja visible el acceso efectivo de la cuenta antes de habilitar operaciones administrativas sobre otros usuarios.
        </p>
      </section>
    </main>
  );
