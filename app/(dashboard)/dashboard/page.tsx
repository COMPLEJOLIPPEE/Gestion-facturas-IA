...
type Vencimiento = { id: string; tipo: "Factura" | "Remito"; numero: string | null; fecha_vencimiento: string; proveedor: string; monto: number; href: string; estado: "vencido" | "proximo" }
...
export default async function DashboardPage() {
  ...
  const [facturasMes, remitosMes, facturasMesAnterior, remitosMesAnterior, facturasPendientes, remitosPendientes, proveedoresActivos, ultimasFacturas, ultimosRemitos, facturasHistorico, remitosHistorico, vencimientosFacturas, vencimientosRemitos, facturaItems, remitoItems] = await Promise.all([
    ...
    supabase.from("facturas").select("id, numero, fecha, total, proveedores (nombre_fantasia)").order("fecha", { ascending: false }).limit(15),
    supabase.from("remitos").select("id, numero, fecha, monto_total, proveedores (nombre_fantasia)").order("fecha", { ascending: false }).limit(15),
  ...
  const vencimientos: Vencimiento[] = [
    ...(vencimientosFacturas.data ?? []).map(f=>{const fechaVencimiento=f.fecha_vencimiento; if(!fechaVencimiento)return null; const saldo=Math.max(Number(f.total??0)-(pagosVF.get(f.id)??0),0); if(saldo<=0)return null; const estado=fechaVencimiento<today?"vencido" as const:"proximo" as const; return{id:f.id,tipo:"Factura" as const,numero:f.numero,fecha_vencimiento:fechaVencimiento,proveedor:(Array.isArray(f.proveedores)?f.proveedores[0]:f.proveedores)?.nombre_fantasia??"—",monto:saldo,href:`/facturas/${f.id}`,estado}}),
    ...(vencimientosRemitos.data ?? []).map(r=>{const fechaVencimiento=r.fecha_vencimiento; if(!fechaVencimiento)return null; const saldo=Math.max(Number(r.monto_total??0)-(pagosVR.get(r.id)??0),0); if(saldo<=0)return null; const estado=fechaVencimiento<today?"vencido" as const:"proximo" as const; return{id:r.id,tipo:"Remito" as const,numero:r.numero,fecha_vencimiento:fechaVencimiento,proveedor:(Array.isArray(r.proveedores)?r.proveedores[0]:r.proveedores)?.nombre_fantasia??"—",monto:saldo,href:`/remitos/${r.id}`,estado}}),
  ].filter((v): v is Vencimiento => { if(!v) return false; const key=`${v.tipo}|${v.id}`; if(seen.has(key)) return false; seen.add(key); return true }).sort((a,b)=>a.fecha_vencimiento<b.fecha_vencimiento?-1:1).slice(0,6)
  const vencimientosColumns: Column<Vencimiento>[] = [
    {key:"tipo",label:"Tipo",render:v=><span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${v.estado==="vencido"?"bg-red-100 text-red-700":"bg-amber-100 text-amber-700"}`}>{v.tipo==="Factura"?"📄 Factura":"📝 Remito"}</span>},
    {key:"numero",label:"Número",render:v=>v.numero??"—"},
    {key:"proveedor",label:"Proveedor"},
    {key:"fecha_vencimiento",label:"Vence",render:v=><span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${v.estado==="vencido"?"bg-red-100 text-red-700":"bg-amber-100 text-amber-700"}`}>{formatDateAR(v.fecha_vencimiento)}{v.estado==="vencido"?" · VENCIDA":" · POR VENCER"}</span>},
    {key:"monto",label:"Saldo pendiente",align:"right",render:v=><span className={`font-semibold ${v.estado==="vencido"?"text-red-600":"text-amber-600"}`}>{formatMoney(v.monto)}</span>},
  ]
  ...
  return <div><div className="mb-8"><h1 className="text-3xl font-bold">Hola 👋</h1><p className="text-gray-600">{user?.email}</p></div>
    <div className="grid gap-6 md:grid-cols-4">
      <div className="rounded-xl border border-blue-200 border-t-4 bg-blue-50 p-6 shadow-sm"><p className="font-medium text-blue-700">💰 Compras del mes</p><h2 className="mt-2 text-3xl font-bold text-blue-950">{formatMoney(comprasMes)}</h2>{variacion!==null&&<p className={`mt-1 text-sm font-medium ${variacion>=0?"text-red-600":"text-green-600"}`}>{variacion>=0?"▲":"▼"} {Math.abs(variacion).toFixed(1)}% vs mes anterior</p>}</div>
      <div className="rounded-xl border border-violet-200 border-t-4 bg-violet-50 p-6 shadow-sm"><p className="font-medium text-violet-700">🧾 Facturas y remitos</p><h2 className="mt-2 text-3xl font-bold text-violet-950">{facturasCount+remitosCount}</h2><p className="mt-1 text-sm text-violet-700/70">{facturasCount} facturas · {remitosCount} remitos</p></div>
      <div className="rounded-xl border border-red-200 border-t-4 bg-red-50 p-6 shadow-sm"><p className="font-medium text-red-700">🔴 Pendientes de pago</p><h2 className="mt-2 text-3xl font-bold text-red-950">{pendientesCount}</h2><p className="mt-1 text-sm text-red-700/80">{formatMoney(pendientesMonto)} de saldo</p></div>
      <div className="rounded-xl border border-emerald-200 border-t-4 bg-emerald-50 p-6 shadow-sm"><p className="font-medium text-emerald-700">🏭 Proveedores activos</p><h2 className="mt-2 text-3xl font-bold text-emerald-950">{proveedoresActivos.count??0}</h2><p className="mt-1 text-sm text-emerald-700/70">proveedores registrados</p></div>
    </div>
