...
export default async function DashboardPage() {
  ...
  const vencimientos: Vencimiento[] = [
    ...(vencimientosFacturas.data ?? []).map(f=>{const saldo=Math.max(Number(f.total??0)-(pagosVF.get(f.id)??0),0); if(saldo<=0)return null; const estado=f.fecha_vencimiento<today?"vencido" as const:"proximo" as const; return{id:f.id,tipo:"Factura" as const,numero:f.numero,fecha_vencimiento:f.fecha_vencimiento as string,proveedor:(Array.isArray(f.proveedores)?f.proveedores[0]:f.proveedores)?.nombre_fantasia??"—",monto:saldo,href:`/facturas/${f.id}`,estado}}),
    ...(vencimientosRemitos.data ?? []).map(r=>{const saldo=Math.max(Number(r.monto_total??0)-(pagosVR.get(r.id)??0),0); if(saldo<=0)return null; const estado=r.fecha_vencimiento<today?"vencido" as const:"proximo" as const; return{id:r.id,tipo:"Remito" as const,numero:r.numero,fecha_vencimiento:r.fecha_vencimiento as string,proveedor:(Array.isArray(r.proveedores)?r.proveedores[0]:r.proveedores)?.nombre_fantasia??"—",monto:saldo,href:`/remitos/${r.id}`,estado}}),
