...
export function FacturaForm({ proveedores, empresas, productos, formasPago }: Props) {
  ...
  const actualizarLinea = (index: number, campo: keyof LineaFactura, valor: string | number) => {
    setLineas((prev) => prev.map((linea, i) => i === index ? { ...linea, [campo]: valor } : linea))
  }

  const actualizarProductoDeLinea = (index: number, productoId: string) => {
    setLineas((prev) => prev.map((linea, i) => i === index ? { ...linea, producto_id: productoId } : linea))
  }
  ...
  const calculo = useMemo(() => {
    const lineasCalculadas = lineas.map((linea) => {
      ...
      const cantidadBonificada = Math.min(
        Math.max(0, numero(linea.cantidad_bonificada ?? linea.cantidad_bonificada_detalle)),
        cantidad
      )
  ...
  const archivoABase64 = (file: File): Promise<string> => new Promise((resolve, reject) => {
  ...
  async function aplicarDatosFacturaIA(datos: Awaited<ReturnType<typeof leerFacturaConIA>>) {
    ...
    if (datos.proveedor_nombre) {
      ...
      const proveedorEncontrado = proveedores.find((proveedor) => {
        const nombreProveedor = proveedor.nombre_fantasia.toLowerCase().trim()
        return nombreProveedor.includes(nombreIA) || nombreIA.includes(nombreProveedor)
      })

      if (proveedorEncontrado) {
        proveedorDetectadoId = proveedorEncontrado.id
        setProveedorId(proveedorEncontrado.id)
      }
    }

    if (datos.lineas.length > 0) {
      const lineasProcesadas = await procesarLineasFacturaConIA(
