    ...
    <div className="rounded-xl bg-white p-6 shadow">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Revise los productos detectados por IA o agregue nuevos manualmente.</p>
        </div>
      ...
      {lineas.length > 0 && (
        <div className="overflow-x-auto">
          <div className="min-w-[1180px]">
            <div className="mb-3 grid grid-cols-[minmax(300px,3fr)_80px_120px_90px_90px_120px_70px_110px_110px_120px_40px] gap-3 border-b pb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">
              <div>Línea / Producto</div><div className="text-center">Cant.</div><div className="text-right">P. Unit.</div><div className="text-right">Dto.</div><div className="text-right">Bonif.</div><div className="text-right">P. Neto Unit.</div><div className="text-center">IVA %</div><div className="text-right">IVA $</div><div className="text-right">Imp. Int.</div><div className="text-right">Total línea</div><div></div>
            </div>
            ...
            {lineas.map((linea, index) => {
              ...
              const precioBrutoUnitario = Number(linea.precio_bruto_unitario ?? precioUnitario);
              const bruto = cantidad * precioBrutoUnitario;
              const descuento = esAjuste ? 0 : Math.abs(Number(linea.descuento ?? 0));
              const bonificacion = esAjuste ? 0 : Math.abs(Number(linea.bonificacion ?? 0));
              ...
              const bonificacionCantidad = esAjuste || linea.tipo_bonificacion !== "cantidad" ? 0 : cantidadBonificada * Math.abs(precioBrutoUnitario);
              const totalBonificacion = bonificacion + bonificacionCantidad;
              ...
              const ivaImporte = linea.iva_importe != null ? Number(linea.iva_importe) : subtotalNeto * (Number(linea.iva ?? 0) / 100);
              const impuestosInternos = Number(linea.impuestos_internos ?? 0);
              const totalLinea = subtotalNeto + ivaImporte + impuestosInternos;

              return (
                <div key={index} className="mb-4 grid grid-cols-[minmax(300px,3fr)_80px_120px_90px_90px_120px_70px_110px_110px_120px_40px] items-start gap-3">
                  <div>
                    {esAjuste ? (
                      <div className="rounded-lg border border-red-200 bg-red-50 p-3">
                        <div className="mb-2 flex items-center justify-between">
                          <span className="rounded-full bg-red-600 px-2 py-1 text-[11px] font-bold text-white">AJUSTE NEGATIVO</span>
                          <button type="button" onClick={() => actualizarLinea(index, "es_ajuste_negativo", false)} className="text-xs font-medium text-red-700 hover:underline">Convertir en producto</button>
                        </div>
                        <p className="text-sm font-semibold text-red-900">{linea.descripcionLeida || "Ajuste del comprobante"}</p>
                        <p className="mt-1 text-xs text-red-700">No requiere seleccionar un producto. El importe y su IVA restan de los totales.</p>
                      </div>
                    ) : (
                      <>
                        ...
                        {!linea.producto_id && !linea.producto_sugerido_id && linea.descripcionLeida && (
                          <button type="button" onClick={() => crearProductoDesdeLinea(index, linea.descripcionLeida ?? "", Math.abs(Number(linea.precio_unitario ?? 0)), Number(linea.iva ?? 21))} className="mt-2 w-full rounded-md bg-green-600 px-3 py-2 text-xs font-semibold text-white hover:bg-green-700">+ Crear este producto</button>
                        )}
                  ...
                  <div className="rounded-lg bg-gray-50 p-2 text-right text-sm">${dinero(descuento)}</div>
                  <div className="rounded-lg bg-gray-50 p-2 text-right text-sm">${dinero(totalBonificacion)}</div>
                  <div className={`rounded-lg p-2 text-right text-sm font-medium ${esAjuste ? "bg-red-100 text-red-800" : "bg-gray-50"}`}>${dinero(precioNeto)}</div>
                  ...
                  <div className={`rounded-lg p-2 text-right text-sm font-medium ${esAjuste ? "bg-red-100 text-red-700" : "bg-blue-50 text-blue-700"}`}>${dinero(ivaImporte)}</div>
                  <div className={`rounded-lg p-2 text-right text-sm font-medium ${esAjuste ? "bg-red-100 text-red-700" : "bg-orange-50 text-orange-700"}`}>${dinero(impuestosInternos)}</div>
                  <div className={`rounded-lg p-2 text-right text-sm font-semibold ${esAjuste ? "bg-red-100 text-red-900" : "bg-gray-100"}`}>${dinero(totalLinea)}</div>
                  <div className="flex justify-center"><button type="button" onClick={() => quitarLinea(index)} className="rounded-lg px-2 py-2 text-red-600 transition hover:bg-red-50 hover:text-red-700">✕</button></div>
                </div>
              );
            })}
          </div>
