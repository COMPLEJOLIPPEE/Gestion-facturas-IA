...
export default async function ProductosPage({
  ...
  if (error) {
    return <Alert variant="error">Error cargando productos: {error.message}</Alert>;
  }
  ...
  return (
    <PageContainer>
      ...
      {writable ? (
        <ProductosTable productos={productos} search={search} estado={estado} categoria={categoria} categorias={categorias} />
      ) : (
        <DataTable columns={columns} data={productos} onView={(p) => `/productos/${p.id}`} />
      )}
    </PageContainer>
  );
}